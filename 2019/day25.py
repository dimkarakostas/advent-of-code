from day25_moves import logged_moves
...
program = [int(i) for i in open('input25').readlines()[0].split(',')]
p = Program(program[:])

# STEP 1: Manually explore the whole space, find all items you can carry, take them to the security check and drop them
manually_explore = False
if manually_explore:
    moves = []
    while False:
        print ''.join(chr(i) for i in p.outputs)
        inp = raw_input()
        move = [ord(i) for i in inp + '\n']
        moves.append(move)
        p.run(move)
        open('day25_moves.py', 'w').write('logged_moves = ' + str(moves))

# STEP 2: Check all item combinations to find the correct one
if not manually_explore:
    items = set([
        'spool of cat6',
    ...
    p.run([i for sublist in logged_moves for i in sublist])
    combs = sum(map(lambda r: list(combinations(items, r)), range(1, len(items) + 1)), [])
    for comb in combs:
        print '[*]', ','.join(comb)
        for item in comb:
            p.run([ord(i) for i in 'take ' + item + '\n'])
        p.run([ord(i) for i in 'south\n'])
        if 'Alert!' not in ''.join(chr(i) for i in p.outputs):
            print ''.join(chr(i) for i in p.outputs)
            break
        for item in comb:
            p.run([ord(i) for i in 'drop ' + item + '\n'])
